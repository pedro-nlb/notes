\documentclass[draft, A4paper, british, reqno]{amsart}
...
\newcommand{\affSch}{\mathbf{Sch}^{\mathrm{aff}}}
\newcommand{\Adic}{\mathbf{Adic}}
...
\begin{nota}
    ...
    \begin{center}
	\begin{tikzcd}
	    p^{-1}(\tau)\arrow{d} \arrow[closed]{r} & \tilde{X} \arrow{d}{p} & \tilde{X}\setminus p^{-1}(\tau) \arrow{d}{\sim}\arrow[open]{l} \\
	    Z_{0}:= X\cap A\arrow[closed]{r} & X & X\setminus Z \arrow[open]{l}
	\end{tikzcd}
    \end{center}
    \begin{center}
	\begin{tikzcd}
	    Z\arrow[closed]{r}{h_{0}} & X_{0}\arrow[closed]{r}{i_{0}} & X \\
	    & & X\setminus Z\arrow[open]{u} \\
	    & & X\setminus X_{0}\arrow[open]{u}
	\end{tikzcd}
    \end{center}
\end{nota}
...
\begin{proof}[Proof of (vi)]
    ...
    \begin{center}
	\begin{tikzcd}
	    H^{k-1}(X_{\infty})\arrow{dr}{h_{\infty}^{*}}\arrow{r}{i_{\infty}^{*}} & H^{k+1}(\tilde{X})\cong H^{k+1}(X)\op H^{k-1}(Z) \arrow{d} \\
	     & H^{k-1}(Z)
	\end{tikzcd}
    \end{center}
...
\begin{proof}[Proof of LHT]
	...
	\item $\tilde{i}_{0}^{*}\colon H^{k}(\tilde{X})\to H^{k}(\tilde{X}_{0})$ is surjective for $k<n-1$.
	    ...
	    \begin{center}
		\begin{tikzcd}
		    & H^{k}(\tilde{X}\setminus \tilde{X}_{\infty})\arrow{dr}{b} & \\
		    H^{k}(\tilde{X})\arrow{ur}{a}\arrow{rr}{i_{0}^{*}} & & H^{k}(\tilde{X}_{0})
		\end{tikzcd}
	    \end{center}
	...
	\item $i_{0}^{*}\colon H^{k}(X)\to H^{k}(X_{0})$ is surjective for $k<n-1$.
	    ...
	    \begin{center}
		\begin{tikzcd}
		    H^{k}(\tilde{X})\arrow[twoheadrightarrow]{d}{(1)}\arrow{r}{\sim} & H^{k}(X)\op H^{k-2}(Z) \arrow[twoheadrightarrow]{d} \\
		    H^(\tilde{X}_{0})=H^{k}(X_{0}) & H^{k-2}(Z)\arrow{l}{h_{0,*}}
		\end{tikzcd}
	    \end{center}
	...
	\item $i_{0}^{*}\colon H^{k}(X)\to H^{k}(X_{0})$ is injective for $k<n-1$.
	    ...
	    \begin{center}
		\begin{tikzcd}
		    H^{k}(X)\arrow{d}{p^{*}}\arrow{r}{i_{0}^{*}} & H^{k}(X_{0})\arrow{dr} & \\
		    H^{k}(\tilde{X})\arrow{r}{\tilde{j}_{\infty}^{*}} & H^{k}(\tilde{X}\setminus X_{\infty})\arrow{r}{\tilde{i}_{0}^{*}} & H^{k}(\tilde{X}_{0}) \\
		    H^{k-2}(X_{\infty})\arrow{u}{\tilde{i}_{\infty,*}} & &
		\end{tikzcd}
	    \end{center}
...
\begin{thm}[Eilenberg-Steenrod]
    ...
    \begin{center}
	\begin{tikzcd}
	    h^{n}(A)\arrow{d}{\varphi_{n}}\arrow{r} & h^{n+1}(X,A)\arrow{d}{\varphi_{n+1}} \\
	    k^{n}(A)\arrow{r} & k^{n+1}(X,A)
	\end{tikzcd}
    \end{center}
...
\begin{rem}
    ...
    \begin{center}
	\begin{tikzcd}
	    H^{*}(X,R)\ot_{R} H^{*}(X,R)\arrow{dr}{\cup}\arrow{r}{\times} & H^{*}(X\times X,R) \arrow{d}{\Delta^{*}} \\
	    & H^{*}(X,R)
	\end{tikzcd}
    \end{center}
\end{rem}
...
\begin{exa}
    ...
    \begin{center}
	\begin{tikzcd}
	    E=p^{-1}(Z)\arrow{d}\arrow{r} & \tilde{X}\arrow{d}{p} \\
	    X\cap A=Z \arrow{r} & X
	\end{tikzcd}
    \end{center}
...
\begin{thm}[Thom isomorphism]
	...
	\begin{center}
	    \begin{tikzcd}
		H^{k}(X\tms B^{r})\arrow{d}{\sim}\arrow{r} & H^{k}(X\times S^{r-1})\arrow{r}\arrow{d}{\sim} & H^{k+1}(X\times B^{r},X\times S^{r-1})\arrow{d}{\sim} \\
		H^{k}(X) \arrow{r} & \op_{p+q=k}H^{p}(X)\ot H^{q}(S^{r-1})\arrow{d}{\sim}\arrow{r} & \op_{p+q=k+1}H^{p}(X)\ot H^{q}(D^{r},S^{r-1})\arrow{d}{\sim} \\
		& H^{k}(X)\op H^{k-r+1}(X) & H^{k-r+1}(X)
	    \end{tikzcd}
	\end{center}
    \end{proof}
\end{thm}
...
\begin{thm}[Tubular neighbourhood]
    ...
    \begin{center}
	\begin{tikzcd}
	    Z\arrow[bend left=45, equal]{rr}\arrow[hook]{r}\arrow[equal]{d} & U\arrow{d}{h}\arrow{r}{r} & Z\arrow[equal]{d} \\
	    Z\arrow{r}{\sigma_{0}}\arrow[equal, bend right=45]{rr} & N_{Z/X}\arrow{r}{\pi} & Z
	\end{tikzcd}
    \end{center}
...
\begin{center}
    \begin{tikzcd}
	& H^{k-2r}(X,X\setminus Z)\arrow{dr} & \\
	H^{k}(Z)\arrow{ur}\arrow{rr}{i_{*}} & & H^{k+2r}(X)
    \end{tikzcd}
\end{center}
commutes.
...
\begin{center}
    \begin{tikzcd}
	\ho(\Top)=\Top/\simeq\arrow{d}{\sim} & \|K\| & X\arrow{d}{\text{sing. chains}} \\
	\ho(\sSet)=\sSet/\simeq & K\arrow{u}{\text{geom. real.}} & \Sing(X)
    \end{tikzcd}
\end{center}
...
\begin{exa}
	...
	\begin{tikzcd}
	    & \varphi(1)\arrow{dr} & \\
	    \varphi(0)\arrow{ur}\arrow{rr} & & \varphi(2)
	\end{tikzcd}
    \end{center}
    If $\bfC$ is a $1$-category, then there is a unique way to "fill-in" the previous triangle, i.e. there is a unique extension
    \begin{center}
	\begin{tikzcd}
	    \Lambda_{1}^{2}=\Delta^{2}\setminus \{0\to 2\} \arrow[hook]{d} \arrow{r} & C \\
	    \Delta^{2}\arrow[dashed]{ur} &
	\end{tikzcd}
    \end{center}
...
\begin{thm}[Cordier-Porter, 1986]
    ...
    \begin{center}
	\begin{tikzcd}
	    \Lambda_{k}^{n}\arrow{r}\arrow[hook]{d} & \bfN_{\Delta}(\bfC)  \\
	    \Delta^{n}\arrow[dashed]{ur}{\exists} &
	\end{tikzcd}
    \end{center}
    for all $0<k<n$.
...
\begin{center}
    \begin{tikzcd}
	\Lambda_{1}^{2}\arrow[hook]{d}\arrow{r} & \bfC \\
	\Delta^{2}\arrow[dashed]{ur} &
    \end{tikzcd}
\end{center}
...
\begin{defn}[Mapping spaces]
    ...
    \begin{center}
	\begin{tikzcd}
	    \Map_{\bfC}(x,y)\arrow{d}\arrow{r} & \bfC_{1} \arrow{d}{(s,t)} \arrow[equal]{r} & \operatorname{hom}(\Delta^{1},\bfC)\arrow{d}{\text{restrict along }\partial \Delta^{1}\hookrightarrow \Delta^{1}} \\
	    *\arrow{r}{(x,y)} & \bfC_{0}\times \bfC_{0}\arrow[equal]{r} & \operatorname{hom}(\partial \Delta^{1},\bfC) 
	\end{tikzcd}
    \end{center}
\end{defn}
...
\begin{exa}
    \begin{center}
	\begin{tikzcd}
	    \bfN(\Cat)\arrow{r}\arrow{dr}{\bfN} & \bfN(\Cat)[eq^{-1}]\arrow[dashed,hook]{d}{fully faithful} \\
	    & \QCat=\infty\text{-cat of q-cats}
	\end{tikzcd}
    \end{center}
\end{exa}
...
\begin{lm}
	...
	\begin{center}
	    \begin{tikzcd}
		p^{-1}\{x\}\arrow{d}{q}\arrow[hook]{r}{\tilde{i}_{x}} & \tilde{X}\arrow{d}{p} \\
		\{ x\}\arrow[hook]{r}{i_{x}} & X
	    \end{tikzcd}
	\end{center}
...
\begin{proof}[Proof of (3)]
    ...
    \begin{center}
	\begin{tikzcd}
	    H^{k}(X,X\setminus Z)\arrow{d}{Thom. iso.}\arrow{r} & H^{k}(\tilde{X},\tilde{X}\setminus E)\arrow{d}{\sim} \\
	    H^{k-4}(Z)\arrow[hook]{r}\arrow{dr} & H^{k-2}(E)\cong H^{k-2}(Z)\arrow{d}{\sim} \\
	    & H^{k-2}(Z)\op H^{k-4}(Z)
	\end{tikzcd}
    \end{center}
    ...
    \begin{center}
	\begin{tikzcd}
	    H^{k}(X,X\setminus Z)\arrow[hook]{d} \arrow{r} & H^{k}(X)\arrow[hook]{d}\arrow{r} & H^{k}(U)\arrow{d}{\sim} \arrow{r} & H^{k+1}(X,X\setminus Z)\arrow[hook]{d} \\
	    H^{k}(\tilde{X},\tilde{X}\setminus E) \arrow{r} & H^{k}(\tilde{X}) \arrow{r} & H^{k}(\tilde{U}) \arrow{r} & H^{k+1}(\tilde{X},\tilde{X}\setminus E)
	\end{tikzcd}
    \end{center}
...
\begin{center}
    \begin{tikzcd}
	& H^{k}(X,X\setminus Z)\arrow{dr}{r} & \\
	H^{k-2r}(Z)\arrow{ur}{\tau_{Z}\cup} \arrow{rr}{i_{*}} & & H^{k}(X)
    \end{tikzcd}
\end{center}
...
\begin{thm}
	...
	\begin{enumerate}
		...
		\begin{center}
		    \begin{tikzcd}
			& H^{k}(\tilde{X}\setminus X_{\infty}) \arrow{dr}{b} & \\
			H^{k}(\tilde{X})\arrow{ur}{a} \arrow[dashed]{rr}{\tilde{i}_{0}^{*}} & & H^{k}(\tilde{X}_{0})
		    \end{tikzcd}
		\end{center}
		...
		\begin{center}
		    \begin{tikzcd}
			H^{k}(\tilde{X})\arrow{r}{\sim}\arrow{d} & H^{k}(X)\op H^{k-2}(Z)\arrow{d} \\
			H^{k}(X_{0}) & H^{k-2}(Z)\arrow{l}{h_{0,*}} 
		    \end{tikzcd}
		\end{center}
		...
		\begin{center}
		    \begin{tikzcd}
			& H^{k}(X)\arrow{rr}{i_{0}^{*}}\arrow{d} & & H^{k}(X_{0})\arrow{d} \\
			H^{k-2}(X_{\infty})\arrow{r}{\tilde{i}_{\infty,*}} & H^{k}(\tilde{X})\arrow{r}{\tilde{j}_{\infty}^{*}} & H^{k}(\tilde{X}\setminus X_{\infty}) \arrow[hook]{r}{(4)} & H^{k}(\tilde{X}_{0})
		    \end{tikzcd}
		\end{center}
		Hence $\tilde{j}_{\infty}^{*}p^{*}(\alpha)=0$.
		...
		\begin{center}
		    \begin{tikzcd}
			H^{k}(\tilde{X})\arrow{ddr}\arrow{r}{\sim} & H^{k}(X)\op H^{k-2}(Z)\arrow[equal]{d} \\
			& \im(p^{*})\op H^{k-2}(Z)\arrow{d} \\
			& H^{k-2}(Z)
		    \end{tikzcd}
		\end{center}
...
\begin{center}
    \begin{tikzcd}
	\P^{2}\arrow[dashed]{rr} & & S \\
	& \tilde{\P^{2}}\arrow{ul}\arrow{ur} & 
    \end{tikzcd}
\end{center}
...
\begin{exa}
    ...
    \begin{center}
	\begin{tikzcd}
	    D((A,A^{+})\usolid)\arrow{d}\arrow{r} & D((\O_{X}(U),\O_{X}^{+}(U))\usolid)\arrow{d} \\
	    D((\O_{X}(V),\O_{X}^{+}(V))\usolid)\arrow{r} & D((\O_{X}(U\cap V),\O_{X}^{+}(U\cap V))\usolid)
	\end{tikzcd}
    \end{center}
    is cartesian.
\end{exa}
...
\begin{prop}[Proposition 10.1]
	...
	\begin{enumerate}
		...
		\begin{center}
		    \begin{tikzcd}
			D((\O_{X}(U),\O_{X}^{+}(U))\usolid)\arrow[hook]{r}{7.5} \arrow{dd}{j_{*}} & D(\Cond(\O_{X}(U)))\arrow[equal]{d} \\
			& D(\Cond(A[\frac{1}{f}]))\arrow[hook]{d}{5.9} \\
			D((A,A^{+})\usolid)\arrow[hook]{r}{7.5} & D(\Cond(A))
		    \end{tikzcd}
		\end{center}
...
\begin{defn}
    ...
    \begin{center}
	\begin{tikzcd}
	    C\arrow{r}{f_{*}}\arrow{d}{p_{*}} & D\arrow{d}{q_{*}}\arrow{dl}{\alpha \text{ htpy }} \\
	    C'\arrow{r}{g_{*}} & D'
	\end{tikzcd}
    \end{center}
...
\begin{prop}[Proposition 10.2]
    ...
    \begin{center}
	\begin{tikzcd}
	    V\arrow[open]{r}\arrow{d} & Y=\Spa(B,B^{+})\arrow{d} \\
	    U\arrow[open]{r}{\text{ratl open}} & X
	\end{tikzcd}
    \end{center}
    Then we have a left-adjointable and cartesian square
    \begin{center}
	\begin{tikzcd}
	    D((\O_{X}(V),\O_{X}^{+}(V))\usolid)\arrow[hook]{r}\arrow{d} & D((B,B^{+})\usolid))\arrow{d} \\
	    D((\O_{X}(U),\O_{X}^{+}(U))\usolid) \arrow[hook]{r} & D((A,A^{+})\usolid)
	\end{tikzcd}
    \end{center}
    \begin{proof}
...
\begin{lm}[Lemma 10.3]
	...
	\begin{enumerate}
		...
		\begin{center}
		    \begin{tikzcd}
			C_{W}\arrow{d}\arrow{r} & C_{U}\arrow{d} \\
			C_{V}\arrow{r} & C
		    \end{tikzcd}
		\end{center}
		...
		\begin{center}
		    \begin{tikzcd}
			C_{U}\cap C_{V}\arrow[hook]{r}\arrow[hook]{d} & C_{U}\arrow[hook]{d} \\
			C_{V}\arrow[hook]{r} & C
		    \end{tikzcd}
		\end{center}
...
\begin{prop}
	...
	\begin{center}
	    \begin{tikzcd}
		H^{2k-1}(X,\Z)\arrow[hook]{r}\arrow[hook]{rrd} & H^{2k-1}(X,\R)\arrow{dr}{\R\text{linear iso }(*)}\arrow[hook]{r} & H^{2k-1}(X,\C)\arrow[twoheadrightarrow]{d} \\
		& & H^{2k-1}(X,\C)/F^{k}H^{2k-1}
	    \end{tikzcd}
	\end{center}
...
\begin{exa}[Jacobians of curves]
    ...
    \begin{center}
	\begin{tikzcd}
	    H_{1}(X,\Z)\arrow{r}\arrow[bend left=45]{rr}{\alpha} & H_{1}(X,\C)\cong H^{1}(X,\C)\dual \arrow{r} & H^{1,0}(X)\dual \\
	    \gamma \arrow[maps to]{rr} & & (\omega \mapsto \int_{\gamma}\omega)
	\end{tikzcd}
    \end{center}
...
\begin{proof}[Proof of Step 2]
    ...
    \begin{center}
	\begin{tikzcd}
	    & k_{\alpha}\arrow{dl}{\chi|_{k_{\alpha}}(h_{3,\alpha},h_{4,\alpha})}\arrow{dr}{\chi|_{k_{\alpha}}(h_{1,\alpha},h_{2,\alpha})} & \\
	    k_{\alpha}\arrow{rr}{g|_{k_{\alpha}}} && k_{\alpha}
	\end{tikzcd}
    \end{center}
...
\section{[CM] Exceptional functors and coherent duality (Brad) - 03.02.20}

\textbf{Last time:}
Globalized $D((A,A^{+})\usolid)$ to $D((\O_{X},\O_{X}^{*})\usolid)$.
Implicitly, we globalized
\[ (-)\ot_{(A,A^{+})\usolid} (B,B^{+})\usolid \dashv \text{forget} \]
to $f^{*}\dashv f_{*}$.

\textbf{Today:}
$f\colon X\to Y$ map of schemes.
\begin{itemize}
    \item $f_{!}\dashv f^{!}$.
    \item $f^{!}\O_{Y}\cong \w_{X/Y}[d]$ for $f$ smooth of relative dimension $d$.
...
\begin{center}
    \begin{tikzcd}
	\affSch_{/R}\arrow[bend left=45]{rr}\arrow[hook]{r}\arrow{d} & \Sch_{/R}\arrow{d}\arrow[hook]{r}{(_)^{ad/R}} & \Adic_{/R}\arrow{d} \\
	\affSch \arrow[bend right=45]{rr} \arrow[hook]{r} & \Sch \arrow[hook]{r}\arrow[Rightarrow]{ur}{\varphi} & \Adic \\
	A \arrow[maps to]{rr} & & \Spa(A,A)
    \end{tikzcd}
\end{center}
For $f\colon X\to Y$ separated finite type, $q\colon X^{ad}\mono X^{ad/Y}$ is an open immersion.
For $f\colon X\to Y$ proper implies $q\colon X^{ad}\xrightarrow{\sim}X^{ad/Y}$ is an isomorphism.
\begin{center}
    \begin{tikzcd}
	X^{ad}\arrow{d}{f^{ad}}\arrow[open]{r}{j} & X^{ad/Y}\arrow{d}{f^{ad/Y}} \\
	Y^{ad}\arrow{r}{\sim} & Y^{ad/Y}
    \end{tikzcd}
\end{center}
because $\id_{Y}$ is proper (analogue of Nagata compactification).

\begin{prop}[Proposition 11.2]
    For all $f\colon X\to Y$ separated finite type map we have an adjunction
    \[ j_{!}\colon D(X^{ad})\Leftrightarrow D(X^{ad/Y})\colon j^{*} \]
    where $D(X^{ad}):=D((\O_{X},\O_{X}^{+})\usolid)$.
    \begin{proof}
	The idea of the proof is (to use Zariski descent) to reduce to $X$, $Y$ affine, and we may even assume f.t. over $\Z$, in which case we apply the results of Lecture 8.
    \end{proof}
\end{prop}

\begin{defn}
    For all $f\colon X\to Y$ sep. f.t. define
    \[ f_{!}:=(f^{ad/Y})_{*}\circ j_{!}\colon D(X^{ad})\to D(Y^{ad}) \]
\end{defn}

\begin{rem}
    \begin{enumerate}
	\item For an open immersion of schemes $j\colon U\mono X$ there is no left adjoint $j_{!}\colon D(U)\to D(X)$ in general.
	    In other words, $j^{*}$ does not preserve limits (in fact, not even products).
	\item $f$ proper implies that $j\colon X^{ad}\to X^{ad/Y}$ is an isomorphism, so $j_{!}\cong j_{*}\cong f^{ad}_{*}=f_{*}$.
    \end{enumerate}
\end{rem}

\begin{prop}
    For all $f\colon X\to Y$ sep. f.t. there exists a right adjoint $f_{!}\dashv f^{!}$.
    \begin{proof}
	We know $j_{!}$ is cocontinuous, as $j_{!}\dashv j^{*}$.
	$f_{*}^{ad/Y}$ cocontinuous: reduce to $X$, $Y$ affine f.t./$\Z$ and apply results from Lecture 8.
	Adjoint functor theorem (infinity category version) implies that we are done.
    \end{proof}
\end{prop}

\begin{fact}
    \begin{enumerate}
	\item For all $X\xrightarrow{f}Y\xrightarrow{g}Z$ composable sep. f.t. maps, we have
	    \[ (gf)_{!}\cong g_{!}f_{!} \quad \text{ and }\quad f^{!}g^{!}\cong (gf)^{!} \]
	\item Porjection formula
	    \[ f_{!}(M\ot_{X^{ad}}f^{*}N)\cong (f_{!}M)\ot_{Y^{ad}}N \]
	\item Proper base change:
	    \begin{center}
		\begin{tikzcd}
		    Y\arrow{r}{g'}\arrow{d}{f'}& X\arrow{d}{f} \\
		    T\arrow{r}{g} & S
		\end{tikzcd}
	    \end{center}
	    if the square is cartesian, then we have
	    \[ g^{*}f_{!}\cong f_{!}'(g')^{*} \quad \text{ and } \quad f^{!}g_{*}\cong g_{*}'(f')^{!} \]
    \end{enumerate}
\end{fact}

\subsection{Dualizing complex}

The goal is to show that if $f\colon R\to A$ is a smooth map of relative dimension $d$, then
\[ f^{!}R\cong \w_{A/R}[d] \]

\begin{prop}[Proposition 11.4]
    Consider the following diagram in which both squares are pushouts:
    \begin{center}
	\begin{tikzcd}
	    R_{0}\arrow{d}{f_{0}}\arrow{r} & R\arrow{d}{f}\arrow{r}{g} & S\arrow{d}{f'} \\
	    A_{0}\arrow{r} & A\arrow{r}{g'} & B
	\end{tikzcd}
    \end{center}
    with $f_{0}$ of f.t. and fin. Tor-dim., $R_{0},A_{0}$ Noetherian and $g$ flat.
    \begin{enumerate}
	\item The composition
	    \[f^{!}R\ot f^{*}M\xrightarrow{\eta}f^{!}f_{!}(f^{!}R\ot f^{*}M)\cong f^{!}((f_{!}f^{!}R)\ot M)\xrightarrow{\varepsilon}f^{!}(R\ot M)=f^{!}M\]
	    is an isomorphism.
	\item The composition
	    \[ (g')^{*}f^{!}\xrightarrow{\eta}(g')^{*}f^{!}g_{*}g^{*}\cong (g')^{*}(g')_{*}(f')^{!}g^{*}\xrightarrow{\varepsilon} (f')^{!}g^{*} \]
	    is an isomorphism.
    \end{enumerate}
    \begin{proof}
	\begin{enumerate}[label=Case \Roman*:]
	    \item $f$ surjective.
		This implies that $\Spec(A)\mono \Spec(R)$ is a closed immersion, hence $f,f'$ are proper, hence $f_{!}\cong f_{*}$ and $f'_{!}\cong f'_{*}$, hence
		\[ f^{!}M\cong R\operatorname{hom}_{R}(A,M)\underset{A \text{ perfect}}{\xrightarrow{\sim}} R\operatorname{hom}_{R}(A,R)\ot^{L}_{A\usolid}M \]
		and similarly for $f'$.
		Now compute.
	    \item $f\colon R\to R[T]=A$ polynomial algebra.
		Then in lecture 8 we computed $f_{!}M=M\ot^{L}_{(A,R)\usolid}(R((T^{-1}))/R[T])[1]$.
		\begin{enumerate}
		    \item 8.13 (iv) in Scholze's notes.
		    \item Use $(1)$ to reduce to $M=R$.
			\begin{align*}
			    (g')^{*}f^{!}R & =R\operatorname{hom}_{R}(R((T^{-1}))/R[T],R)[1]\ot^{L}_{A\usolid} B\usolid \\
			    & \cong R[T][1]\ot^{L}_{A\usolid}B\usolid \\
			    & \cong A[1]\ot^{L}_{A\usolid}B\usolid \\
			    & \cong B[1]
			\end{align*}
			\begin{align*}
			    f'^{!}g^{*}R &\cong f'^{!}S & \quad \text{(g flat)} \\
			    & \cong R\hom_{S}(S((T^{-1}))/S[T],S)[1] & \\
			    & \cong S[T][1] & \\
			    & \cong B[1] &
			\end{align*}
		\end{enumerate}
...
\begin{prop}[Prop. 11.5]
    Let $f\colon R\to A$ be a surjective map of rings.
    Let $I$ be its kernel, which we assume to be generated by a regular sequence $f_{1},\ldots,f_{c}$, so that the corresponding map of schemes $\Spec(A)\mono \Spec(R)$ is a regular closed immersion of codimension $c$.
    Then $f^{!}R\cong R\operatorname{hom}_{R}(A,R)\cong \det_{A}(I/I^{2})\dual [-c]$.
    \begin{proof}
	Koszul complex yields $R\operatorname{hom}_{R}(A,R)\overset{1}{\cong} A[-c] $.
	\begin{align*}
	    A &\overset{2}{\xrightarrow{\sim}} \det_{A}(I/I^{2}) \\
	    1 & \mapsto f_{1}\wedge \cdots \wedge f_{c}
	\end{align*}
	$f$ closed immersion implies that $f_{!}\cong f_{*}$, so $f^{!}R\cong R\operatorname{hom}_{R}(A,R)$.
	\begin{align*}
	    f^{!}R & \overset{3}{\cong} R\operatorname{hom}_{R}(A,R) \\
	    & \overset{1}{\cong} A[-c] \\
	    & \cong A\dual[-c] \\
	    & \overset{2}{\cong} \det_{A}(I/I^{2})\dual [-c]
	\end{align*}
    \end{proof}
\end{prop}

\begin{thm}[Thm. 11.6]
    Let $f\colon R\to A$ be a smooth ring map of relative dimension $d$.
    Then 
    \[ f^{!}R\cong \det_{A}(\Omega^{!}_{A/R})[d] \]
    \begin{proof}
	Let $g\colon A\ot_{R}A\to A$ be the multiplication and $p,q\colon A\to A\ot_{R} A$ sending $a$ to $a\ot 1$ and $1\ot a$ respectively.
	Note that $g$ is a regular closed immersion of codimension $d$.
	$gp=\id_{A}=gq$.
	We have
	\begin{align*}
	    f^{!}R & \cong g^{!}p^{!}f^{!}R & \quad \text{(since } gp=\id_{A}\text{)} \\
	    & \cong g^{!}(p^{*}f^{!}R\ot_{A\ot_{R}A}p^{!}A) & \quad \text{(by } 11.4.(1)\text{)} \\
	    & g^{!}(p^{*}f^{!}R\ot_{A\ot_{R}A}q^{*}f^{!}R) & \quad \text{(by } 11.4.(2), p!A=p^{!}f^{*}R\text{)} \\
	    & \cong f^{*}(p^{*}f^{!}R\ot_{A\ot_{R}A}q^{*}f^{!}R)\ot_{A} g'(A\ot_{R}A) & \quad \text{(by } 11.4.(1)\text{)} \\
	    & f^{!}R\ot_{A\ot_{R}A}f^{!}R\ot_{A}g^{!}(A\ot_{R}A) & \quad \text{(}g^{*} \text{ monoidal)}
	\end{align*}
	Factor $f$ as $R\xrightarrow{a}R[T_{1},\ldots,T_{n}]\overset{b}{\epi}A$.
	$f^{!}R\cong b^{!}a^{!}R$.
	Lecture 8: $a^{!}R$ is $\ot$-invertible.
	11.5 implies that $b^{!}a^{!} $ is $\ot$-inverstibel as well.
	Hence
	\[ f^{!}R\cong f^{!}R\ot f^{!}R\ot g'(A\ot_{R}A)\]
	Must show $f^{!}R\cong (g^{!}(A\ot_{R}A))\dual $.
	11.5 implies $g^{!}(A\ot_{R}A)\cong \det_{A}(I/T^{2})\dual[-d]$ with $I$ the kernel of the multiplication.
	Then
	\[ I/I^{2}\cong \Omega^{1}_{A/R} \]
    \end{proof}
\end{thm}
...
\begin{rem}
    $f_{*}$ preserve discrete objects.
    (for $X$, $Y$ smooth.)
    \begin{center}
	\begin{tikzcd}
	    D(X)\arrow{r}{f_{*}}\arrow{d}{\sim} & D(Y)\arrow{d}{\sim} \\
	    \operatorname{Ind}(\mathbf{Perf}(X))\arrow[hook]{d} & \operatorname{Ind}(\mathbf{Perf}(Y))\arrow[hook]{d} \\
	    D(X^{ad})\arrow{r}{f_{*}}& D(Y^{ad})
	\end{tikzcd}
    \end{center}
\end{rem}

\begin{rem}
    $f_{!}$ preserves compact objects (follows from lecture 8).
\end{rem}

\begin{rem}
    $M\in D(X^{ad})$ perfect iff discrete and compact, so if $f$ is proper, then $f_{!}\cong f_{*}$ preserves perfect objects.
    By adjunction, $f$ proper implies that $f^{!}$ agrees with classical construction.
\end{rem}

\subsection{Grothendieck's formulation of coherent duality}

\begin{center}
    \begin{tikzcd}
	X\arrow{rr}{f\text{ proper}}\arrow{dr}{\pi_{X}} & & Y\arrow{dl}{\pi_{Y}} \\
	& \Spec(R) &
    \end{tikzcd}
\end{center}
with $\pi_{X},\pi_{Y}$ smooth.
Then $f_{*}\dashv R\operatorname{hom}(f^{*}R\hom(-,\w_{Y/R}[d_{Y}]),w_{X/R}[d_{X}])$, where $f_{*}\colon D^{b}_{c}(X)\to D^{b}_{c}(Y)$.

\[ [M,R\operatorname{hom}(f^{*}R\operatorname{hom}(N,\w_{Y}[d_{Y}]),\w_{X}[d_{X}])]\cong \]
\[ \cong [M,R\operatorname{hom}(f^{*}R\operatorname{hom}(N,\pi_{Y}^{!}R),\pi_{X}^{!}R)] \quad (\text{11.6}) \]
\[ \cong [M\ot f^{*}R\operatorname{hom}(N,\pi_{Y}^{!}R),\pi_{X}^{!}R] \quad (\ot \dashv \operatorname{hom} ) \]
\[ \cong [M\ot f^{*}R\operatorname{hom}(N,\pi_{Y}^{!}R),f^{!}\pi_{Y}^{!}R] \quad (\pi_{X}=\pi_{Y}f ) \]
\[\cong [f_{!}(M\ot f^{*}R\operatorname{hom}(N,\pi_{Y}^{!}R)),\pi_{Y}^{!}R] \quad (f_{!}\dashv f^{!}) \]
\[ \cong [(f_{!}M)\ot R\operatorname{hom}(N,\pi_{Y}^{!}R),\pi_{Y}^{!}R] \quad \text{ projection formula} \]
\[ \cong [f_{!}M,R\operatorname{hom}(R\operatorname{hom}(N,\pi_{Y}^{!}R),\pi_{Y}^{!}R)] \quad \ot \dashv \operatorname{hom} \]
\[ \cong [f_{!}M,N] \quad N \text{ } \ot \text{-dualizable, }\pi_{Y}^{!}R \text{ } \ot \text{-dualizable.} \]
\[ [f_{*}M,N] \quad f_{!}\cong f_{*} \]

\section{[WS] Jacobians 4 (Vincent) - A primer on Abelian Varieties - 05.02.20}

References: C.~Birkenhake, H.~Lange--Complex Abelian Varieties.
D.~Mumford--Prym varieties.

\subsection{Abelian varieties}

\begin{defn}
    Let $g\geqslant 1$.
    Let $X=\C^{g}/\Lambda$, where $\Lambda \sub \C^{g}(=V)$ is a full lattice.
    Then $X$ is called a \textit{torus}.
\end{defn}

\begin{q}
    When is $X$ projective?
\end{q}

This is the case iff $X$ admits a holomorphic ample line bundle, iff $X$ admits a holomorphic positive definite line bundle.

$\Pic(X)\cong H^{1}(X,\O_{X}^{\tms})$ which fits into the LES coming from the exponential sequence.
\begin{center}
    \begin{tikzcd}
	\Pic(X)\arrow{r}{\partial}\arrow{dr}{c_{1}} & H^{2}(X,\Z)\cong \Lambda^{2} H^{1}(X,\Z) \\
	& \operatorname{NS}(X)=\im(\partial)\arrow[hook]{u}
    \end{tikzcd}
\end{center}
The guy on the top--right corner is isomorphic to $\Hom(H_{1}(X,\Z),\Z)\cong \pi_{1}(X,\Z)\cong \Lambda$, which in turn is isomorphic to $\operatorname{Alt}^{2}(\Lambda,\Z)\sub \operatorname{Alt}^{2}_{\R}(V,\R)$.

\begin{fact}
    $E\in \operatorname{Alt}^{2}(V,\R)$ is of the form $c_{1}(L)$ for some $L\in \Pic(X)$ iff the following conditions hold:
    \begin{enumerate}
	\item $E(\Lambda,\Lambda)\sub \Z$.
	\item $E(iv,iw)=E(v,w)$ for all $v,w\in V$.
    \end{enumerate}
\end{fact}

We have associated to each line bundle an alternating form with the previous propreties, but we are not quite done yet.

\begin{fact}
    There is a one--to--one correspondence between $E\in \operatorname{Alt}^{2}(V,\R)$ satisgyin $(2)$ and hermitian forms given by
    \[ E\mapsto H(v,w)=E(iv,w)+iE(v,w) \]
    and the inverse is given by $H\mapsto \Im(H)$.
\end{fact}

Hence we have associated an hermitian form $H_{L}\cong c_{1}(L)$ to $L$.

\begin{defn}
    We say that $L$ is a positive definite line bunddle if $H$ is positive definite.
\end{defn}

\begin{thm}
    $L$ is ample iff $L$ is positive definite.
\end{thm}

\begin{defn}
    A choice of such a positive definite $H_{L}$ coming from some $L\in \Pic(X)$ is called a \textit{polarization}.

    A torus admitting such a polarization is called an \textit{abelian variety}.

    $E_{L}|_{\Lambda}\cong \begin{pmatrix} 0 & D \\ -D & 0 \end{pmatrix}$ in a certain basis of the lattice $A$ where $D=\operatorname{diag}(d_{1},\ldots,d_{g})$, with $d_{i}| d_{i+1}$.
    The sequence $(d_{1},\ldots,d_{g})$ is called the \textit{type} of $L$.
    
    $(X,L)=(X,c_{1}(L))$ is a principal polarization if $L$ is of type $(1,\ldots,1)$.
    Any divisor $\Theta$ such that $c_{1}(\O_{C}(\Theta))=c_{1}(L)$ is called a \textit{theta divisor}.
\end{defn}

\begin{exa}
    Every torus of dimension $g=1$ is an abelian variety.
    $X=\C/\Lambda$.
    WLOG $\Lambda=\lbrace 1,2\rbrace$ with $\Im(z)>0$.
    \begin{align*}
	H\colon \C\times \C &\to \C \\
	(v,w) & \mapsto \frac{v\cdot \bar{w}}{\Im(z)}
    \end{align*}
    Recall that we have to check integrality on the imaginary part only.
\end{exa}

\subsection{Jacobians}

$C$ a smooth projective curve of genus $g$.
We associate to it the torus $J(C)=\frac{H^{0}(\w_{C})\dual}{H_{1}(X,\Z)}$.

Fix a basis $\lambda_{1},\ldots,\lambda_{2g}$ of $H_{1}(C,\Z)$ such that the intersection pairing w.r.t. this basis is of the form
\[ A=\begin{pmatrix} 0 & -1_{g} \\ 1_{g} & 0 \end{pmatrix}, \]
hence $\lambda_{i}$ intersects only $\lambda_{i+g}$.

So $H^{0}(\w_{C})\dual $ has alternating form induced from $A^{-1}=\begin{pmatrix} 0 & 1_{g} \\ -1_{g} & 0 \end{pmatrix}$, hence $H$ hermitian.

\begin{prop}
    $H$ is a positive definite hermitian form coming from some $L$ on $J(C)$.
    $H$ is of type $(1,\ldots,1)$ (or equivalently $H$ corresponds to a principal polarization).
\end{prop}

\subsection{Abel--Jacobi theorem}

There is a bijection between $\Pic^{0}(C)$ and $J(C)$ sending $D=\sum_{i}(P_{i}-Q_{i})$ to
\[ w\mapsto \sum_{\nu}\int_{Q_{\nu}}^{P_{\nu}}w \]
$\Pic^{d}(C)$ is a $\Pic^{0}(C)$-torsor.
For any $L_{0}\in\Pic^{d}(C)$ we have
\begin{align*}
    \Pic^{d}(C) & \xrightarrow{\alpha_{L_{0}}} \Pic^{0}(C)\cong J(C) \\
    L &\mapsto L\ot L_{o}^{-d}
\end{align*}

\subsection{Theta divisors}

$C$ a curve of genus $g\geqslant 4$, $\Theta $ a divisor.
Then
\[ \dim{\operatorname{sing}(\Theta)}=
\begin{cases}
    g-4 & \text{ if } C \text{ is not hyperelliptic} \\
    g-3 & \text{ otherwise}
\end{cases}
\]

\textbf{Next talk:}
Intermediate Jacobian of smooth cubic threefold is a \textit{Prym variety}.

If smooth cubic threefold was rational, then intermediate Jacobian would be a product of Jacobians.
Hence to show that the cubic $3$ fold is not rational we want to compute the singular locus of theta divisors on Prym variety and Jacobians.

\textbf{Recall:}
$J(C)=\Pic^{0}(C)$ comes equipped with a principal polarization, i.e. a Chern class $c_{1}(L)$.

Any divisor $D$ s.t. $c_{1}(\O_{C}(D))=c_{1}(L)$ is called a \textit{theta divisor}.

\begin{fact}
    Any two theta divisors are translates of each other.
\end{fact}

The singular locus of $\Theta $ is the set of points with multiplicity at least 2.

\begin{defn}
    A curve $C$ of genus $g\geqslant 2$ is called \textit{hyperelliiptic} if there is a finite morphism $f\colon C\to \P^{1}$ of degree $2$.
    $\iota \colon C\to C$ exchanging the sheets of $f$ is called a \textit{hyperelliptic involution}.
\end{defn}
\begin{fact}
    $f$ is unique if it exists an it is induced from $\O_{C}(P+\iota(P))$ for any $P\in C$.
\end{fact}

\begin{prop}
    $C$ is hyperelliptic iff $\varphi_{\w_{C}}\colon C\to \P^{g-1}$ is not an embedding.
\end{prop}

\begin{prop}
    We can factor
    $\varphi_{\w_{C}}\colon C\xrightarrow{f}\P^{1}\overset{\text{Ver}}{\mono} \P^{g-1}$.
    Then $\varphi_{\w_{C}}(P)=\varphi_{\w_{C}}(Q)\Leftrightarrow f(P)=f(Q)$.
\end{prop}

\textbf{Recall:}
$C^{(g-1)}=C^{g-1}/S_{g-1}\xrightarrow{\rho} \Pic^{g-1}(C)\xrightarrow{\ot L_{0}^{-1}} \Pic^{0}(C)\cong J(C)\supseteq \Theta$.
$L_{0}$ is of degree $g-1$ and the tensor map is an isomorphism.

\begin{thm}[Riemann]
    For certain $\Theta$ and $L_{0}\in \Pic^{g-1}(C)$:
    \[ \alpha_{L_{0}}^{*}(\Theta)=\rho(C^{(g-1)})=W_{g-1}=\{ L\in \Pic^{g-1}(C)\mid h^{0}(L)\geqslant 1\} \]
    $\rightsquigarrow $ calculate dimension of singular locus of $W_{g-1}$.
\end{thm}

\begin{thm}[Riemann Singularity Theorem]
    $\operatorname{sing}(W_{g-1})=\{L\in \Pic^{g-1}(C)\mid h^{0}(L)\geqslant 2\}$
    and $\operatorname{mult}_{L}(\Theta)=h^{0}(L)$.
\end{thm}

\begin{thm}[Geometric Riemann-Roch]
    For any divisor we have
    \[ h^{0}(\O_{C}(D))=\deg(D)-\dim(\operatorname{span}(\varphi_{\w_{D}}(D))) \]
    where the span is the intersection of all hyperplanes containing $\varphi_{\w_{C}}(D)$.
\end{thm}

Now we want to prove

\begin{prop}
    Let $C$ be a curve of genus $g\geqslant 4$.
    Then for any theta diviso $\Theta$ on $J(C)$ we have
    \[ \dim{\operatorname{sing}(\Theta)}=
    \begin{cases}
	g-4 & \text{ if } C\text{ is not hyperelliptic} \\
	g-3 & \text{ otherwise}
...
\subsection{Prym varieties}

Let $\pi\colon \tilde{C}\to C$ be an etale double cover of degree $2$ between smooth projective curves.
Consider the involution $\iota\colon \tilde{C}\to \tilde{C}$ exchanging the sheets.
We have $J(\tilde{C})=\Pic^{0}(\tilde{C})$ and $J(C)=\Pic^{0}(C)$.
From $J(\tilde{C})$ to $J(C)$ we can go via $\pi_{*}=\operatorname{Nm}$.
From $J(C)$ to $J(\tilde{C})$ we can go via $\pi^{*}$.

\begin{fact}
    \begin{enumerate}
	\item $\iota_{*}\colon J(\tilde{C})\to J(C)$.
	\item $\operatorname{Nm}\circ \pi^{*}$ is multiplication by $2$.
	\item $\operatorname{Nm}$ is surjective.
	\item $\pi^{*}\operatorname{Nm}(D)= D+\iota_{*}(D)$
	    Hence $\iota_{*}(\pi^{*}(D))=\iota_{*}(\pi^{*}\operatorname{Nm}(D))=\iota_{*}(D'+\iota_{*}(D'))=\pi^{*}(D)$.
	    Hence $\pi^{*}(J(C))=\im(1-\iota)$ is invariant under $\iota$.
    \end{enumerate}
\end{fact}

$\calP=\im(1-\iota)$ is called \textit{Prym variety}.
Get $J(\tilde{C})\cong J(C)\tms P/H$, where $H$ is some finite group.

\begin{fact}
    There is a principal polarization on $\calP$.
    There exists a theta divisor $\Theta$ on $J(C)$ and $\Xi$ on $\calP$ such that
    \[ \Theta|_{\calP}=2\Xi \]
\end{fact}

\begin{prop}
    The singular locus of $\Xi$ can be described as 
    \[ \{ x\in \calP\mid \operatorname{mult}_{X}(\Theta)\geqslant 4\}\cup \{x\in \calP\mid \operatorname{mult}_{x}(\Theta)=2 \text{ and }T_{x,\calP}\sub TC_{x}\Theta \} \]
\end{prop}

\section{[FS] Stefan Schreieder - 07.02.20}

\subsection{Motivation}

$k=\bar{k}$ alg. closed field, $X$ a variety over $k$, today $X=X_{d}\sub \P_{k}^{n+1}$ hypersurface of degree $d$, i.e. $X_{d}=\{f=0\}$, $f$ irred. hom. of deg $d$.
Some properties we want to look at are the following:

(R) $X$ is rational if there exists $\P^{n}\overset{\sim}{\dashrightarrow} X$ birational map, which is equivalent to both varieties having isomorphic dense open subsets, which in turn is equivalent to almost all solutions of $f=0$ being parametrized $1:1$ by $t\in\P^{n}$.

(UR) $X$ is unirational if $\exists \P^{n}\overset{(r:1)}{\dashrightarrow} X$ dominant rational map, which is equivalent to almost all solutions $f=0$ are parametrized $r:1$ by $t\in \P^{n}$.

(RC) $X$ is rationally connected if for all general points $x,y\in X$ there exists $\P^{1}\xrightarrow{\varphi} X$ such that $x,y\in \im(\varphi)$.

\begin{fact}
    (R) $\Rightarrow $ (UR) $\Rightarrow $ (RC).
\end{fact}

It is known that the left implication cannot be reverted, and it is conjectured that the right one cannot as well.

\begin{exa}
    \begin{enumerate}
	\item $\{ x_{0}^{2}+x_{1}^{2}=x_{2}^{2}\}\sub \P^{2}$ is rational because almost all solutions of $x_{0}^{2}+x_{1}^{2}=1$ are parametrized $1:1$ by
	    \[ x_{0}=\frac{t^{2}-1}{t^{2}+1} \quad \text{and}\quad x_{1}=\frac{2t}{t^{2}+1}. \]
	    More generally, any $X_{2}\sub \P^{n+1}$ is rational.
	\item (Moran, Harris-M-Pandharipande, B-R) $X_{d}\sub \P^{n+1}_{\C}$ smooth with $d!\leqslant \log_{2}(n+1)$ is unirational.
	\item $X_{d}\sub \P_{\C}^{n+1}$ smooth of degree $d\leqslant n+1$ is Fano, so by [KMM, Campana], $X_{d}$ is (RC).
	    This completes the case of hypersurfaces with respect to (RC), because we also know that for higher degrees the corresponding smooth hypersurfaces are not (RC).
    \end{enumerate}
\end{exa}

\begin{conj}
    $X_{d}\sub \P_{\C}^{n+1}$ very general hypersurface (i.e. $[f]\in \P(H^{0}(\P^{n+1},\O(d)))$ lies outsie some countable union of closed subsets\footnote{Morally we can think of this as the coefficients of $f$ being algebraically independent, although it is not equivalent}).
    If $n\geqslant 3$ and $d\geqslant 3$, then $X_{d}$ is not (R).
\end{conj}

\begin{conj}
    $X_{d}\sub \P^{n+1}_{\C}$ very general with $n\geqslant 3$, $1<<d\leqslant n+1$.
    Then $X_{d}$ is not (UR).
\end{conj}
...
\begin{thm}[Kollar '95]
    Conjecture 1 is true for $d\geqslant 2\lceil \frac{n+3}{3}\rceil$.
\end{thm}

\begin{thm}[Totaro '15]
    Conjecture 1 is true for $d\geqslant 2\lceil \frac{n+2}\rceil $.
\end{thm}

Conjecture 2 is open but Kollar et al prove some bounds on the degree of $\phi\colon \P^{n}\dashrightarrow X_{d}$.

New results:

\begin{thm}[Thm A, Sch. 18,19]
    $k=\bar{k}$ uncountable.
    $X_{d}\sub \P_{k}^{n+1}$ very general, $n\geqslant 3$.
    If $\exists \phi\colon \P^{n}\dashrightarrow X_{d}$ (UR) parametrization (dominant), then $m\mid \deg(\phi)$ for all $m\leqslant d-\log_{2}(n), \frac{1}{m}\in k$.
\end{thm}

\begin{cor}
    If $m=2$, characteristic of $k$ different from $2$, $X_{d}\sub \P^{n+1}$ very general is irrational if $d\geqslant \log_{2}(n)+2$.
    Fr $m=3$ and characteristic 2, $X_{d}\sub \P^{n+1}$ very general is irrational if $d\geqslant \log_{2}(n)+3$.
\end{cor}

For big numbers this improves the previous bounds a lot.

\subsection{Torsion orders}

$X$ variety over $k$.
Consider $\CH_{0}(X)$ (free ab. group on closed points modulo rational equivalence).

\begin{exa}
    If $X$ is smooth over $k=\bar{k}$ is (RC), then $\CH_{0}(X)$ is $\Z$ (iso. given by degree), because any two points in $\P^{1}$ are linearly equivalent.
\end{exa}

\begin{rem}
    The example is not true if $k\neq \bar{k}$.
    In fact, if $X/k=\bar{k}$ is (RC), it is not true that $\deg\colon \CH_{0}(X_{L})\xrightarrow{\sim} \Z$ for all $L\supseteq k$.
    But $X_{\bar{L}}$ is (RC) and has such an isomorphism.
    \[ \ker(\CH_{0}(X_{L})\to \CH_{0}(X_{\bar{L}})) \quad \text{ is torsion.}\]
\end{rem}
This torsion statement is true in very general situation, it relies only on "push and pull argument".

\begin{defn}
    The \textit{torsion order of $X$}, denoted $\Tor(X)$, is the smallest integer $e\in \Z_{>}\cup \{\infty\}$ such that
    \[ \ker(\CH_{0}(X_{L})\xrightarrow{\deg}\Z) \]
    is killed by $\cdot e$ for all $L/k$.
\end{defn}

\begin{lm}
    $\Tor(X)$ is the order of $\delta_{X}-z_{k(X)}\in \CH_{0}(X_{k(X)})$, where $\delta_{X}$ is the point of diagonal and $z\in X(k)$ is base change of $z$.
    ($X$ smooth and proper).
\end{lm}

\begin{lm}
    \begin{enumerate}[label=\alph*)]
	\item $f\colon X\dashrightarrow Y$ dominant implies that $\Tor(Y)\mid \deg(f)\Tor(X)$.
	\item If $X\times \P^{m}$ is rational, then $\Tor(X)=1$.
    \end{enumerate}
\end{lm}

\begin{cor}
    If $\phi\colon \P^{n}\dashrightarrow X$, then $\Tor(X)\mid \deg(\phi)$.
\end{cor}

Hence Theorem A follows from:

\begin{thm}[Theorem B]
    $X_{d}\sub \P_{k}^{n+1}$ very general over $k=\bar{k}$, $n\geqslant 3$.
    Then $m\mid \Tor(X_{d})$ if $m\leqslant d-\log_{2}(n)$ and $\frac{1}{m}\in k$.
\end{thm}

The bound gets close to previous universal bound:

\begin{thm}[Roitman '70s]
    $X_{d}\sub \P_{k}^{n+1}$ smooth over any field.
    Then $\Tor(X_{d})\mid d!$.
\end{thm}
We should not expect this universal bound to be sharp in all cases (e.g. quadrics without rational points in characteristic $2$), and Thm. B is for $k=\bar{k}$.

\subsection{Unramified cohomology}

$Y$ a variety over $k=\bar{k}$, $\frac{1}{m}\in k$, then
\[ H^{i}(k(Y),\Z/m):=\varinjlim_{\varnothing \neq U\sub Y}H^{i}(U,\Z/m) \left( = H^{i}(\operatorname{Gal}_{k(Y)},\Z/m)\right) \]
It is a birational invariant, but it is huge (does not even see the genus).
So instead:

\begin{defn}[Unramified cohomology]
    \[ H_{nr}^{i}(Y,\Z/n):=\{\alpha\in H^{i}(k(Y),\Z/m) \mid (*) \} \]
\end{defn}

$(*)$ For any normal birational model $Y'$ of $Y$, $\alpha$ lifts to a "big" open subset $U'\sub Y'$, where "big" means its complement has codimension at least $2$.

\begin{exa}
    \begin{enumerate}
	\item $H^{1}_{n r}(Y, \Z/n)=H^{1}(Y,\Z/n)$.
	\item $H^{i}_{n r}(Y,\Z/n)=0$ if $\dim{Y}<i$.
    \end{enumerate}
\end{exa}

\begin{fact}
    This is a stable birational invariant and $\ord(\gamma)\mid \Tor(Y)$ for all $\gamma \in H^{i}_{nr}(Y,\Z/m)$.
\end{fact}

Issue: hard to compute, and often vanishes when computable.

\subsection{Specialization}

Need a stronger version of a previous result by Voisin, CT-Pirutha.

\begin{thm}[Sch. 18]
    $\X\to C$ flat projective family/ $k=\bar{k}$, $C=$ smooth curve.
    $X$ a very general fibre (the geometric generic fibre), $Y$ a special fibre.
    Assume:
    
    (O) there exists $\gamma \in H^{i}_{nr}(Y,\Z/m)$ of order $m$.

    (V) there exists a resolution (alteration) $\tau\colon Y'\to Y$ such that for all $E\sub \tau^{-1}(Y^{sing})$ we have
    \[ \gamma|_{E}=0\in H^{1}(k(E),\Z/n) \]
    Then $m\mid \Tor(X)$.
\end{thm}

\section{[WS] Rationality 5 (Johan) - Counterexamples to the Lueroth problem - 12.02.20}

\textit{Reminder:}
$X$ smooth projective variety is rational iff there exists $\P^{n}\dashrightarrow X$ birational, and unirational if there exists $\P^{n}\dashrightarrow X$ dominant.
We showed that all cubic threefolds are unirational.

The Lueroth problem was then: is every unirational variety rational?

\textit{Today:}
In dimension $3$, the answer is no.
The specific plan is:
\begin{enumerate}[label=\roman*)]
    \item Review ppav and intermediate Jacobians.
    \item If $X$ is a rational threefold, then $J(X)$ is a product of Jacobians of curves.
    \item Show for an explicit cubic threefold $X$ that the Jacobian is not of that form.
    \item Try the general case.
\end{enumerate}

\subsection{Polarised abelian varieties and (intermediate) Jacobians}

Let $(A_{1},D_{1})$ and $(A_{2},D_{2})$ be two polarised abelian varieties.
Then there is a natural polarisation on the product $(A_{1}\times A_{2},p_{1}^{*}D_{1}+p_{2}^{*}D_{2})=(A_{1}\times A_{2},(D_{1}\times A_{2})+(A_{1}\times D_{2}))$.

\begin{defn}
    $(A,D)$ is called \textit{indecomposable} if it is nontrivial and it cannot be written as a product of two nontrivial polarised abelian varieties.
\end{defn}

Let $D$ be any divisor on an abelian variety $A$.
Write $D_{a}$ for $D+a$, the translate of $D$ by some element $a\in A$.
We get a map
\begin{align*}
    \varphi_{D}\colon A &\longrightarrow \hat{A} \\
    a &\longmapsto \O(D_{a}-D)
\end{align*}

\begin{rem}
    If $D$ is a principal polarisation, then $\varphi_{D}$ is an isomorphism.
\end{rem}

\begin{thm}
    \begin{enumerate}[label=\roman*)]
	\item A nontrivial principally polarised abelian variety $(A,D)$ is indecomposable iff $D$ is irreducible and reduced.
	\item Every ppav $(A,D)$ has a unique decomposition as product $(A_{1},D_{1})\times \cdots \times (A_{n},D_{n})$.
    \end{enumerate}
    \begin{proof}[Sketch of proof]
	If $(A,D)=(A_{1},D_{1})\times \cdots \times (A_{n},D_{n})$, then $D=D^{(1)}+\cdots +D^{(n)}$, with $D^{(i)}=A_{1}\times \cdots \times D_{i}\times \cdots \times A_{n}$.
	We can recover $A_{i}=\varphi_{D}^{-1}(\varphi_{D^{(i)}})$.
	Let $(A,D)$ be any ppav.
	Write $D=D^{(1)}+\cdots +D^{(n)}$ as a sum of irreducible components.
	Put $A_{i}=\varphi_{D}^{-1}(\varphi_{D^{(i)}}(A))$ and $D_{i}=D^{(i)}|_{A_{i}}$.
	One can show that $(A,D)=(A_{1},D_{1})\times \cdots \times (A_{n},D_{n})$.
    \end{proof}
\end{thm}

\begin{cor}
    The Jacobian of a curve $J(C)$ is indecomposable.
    \begin{proof}
	Last week we saw that $\Theta_{J(C)}$ is the image of
	\begin{align*}
	    C^{(g-1)} &\longrightarrow J(C) \\
	    D &\longmapsto D-(g-1)[x]
	\end{align*}
	for some fixed $x\in C$.
    \end{proof}
\end{cor}

\begin{defn}
    Let $X$ be a smooth projective $n$-dimensional variety and $k\in \N$.
    We defined
    \[ J^{k}(X)=\frac{H^{2k-1}(X,\C)}{F^{k}H^{2k-1}(X,\C)+H^{2k-1}(X,\Z)}=\frac{F^{n-k+1}H^{2n-2k+1}(X,\C)\dual}{H_{2n-2k+1}(X,\Z)}, \]
    where $F^{q}H^{i}(X,\C)=\op_{p\geqslant q}H^{p,i-p}(X)=\op_{p\geqslant q}H^{i-p}(X,\Omega^{p})$.
\end{defn}

\subsection{Rational threefolds}

\begin{prop}[Blow-up formula]
    Let $X$ be a smooth projective variety, $Y\sub X$ closed and smooth subvariety of codimension $c$.
    Let $\tilde{X}$ be the blow-up of $X$ along $Y$.
    Then there are natural isomorphisms
    \[ H^{p}(\tilde{X},\Z)\cong H^{p}(X,\Z)\bigoplus \left(\op_{k=1}^{c-1}H^{p-2k}(Y,\Z)\right) \]
\end{prop}

Blowing up a point (resp. a genus $g$ curve) on a threefold yields:
\begin{center}
    \begin{tikzcd}
	&[-25pt] &[-25pt] &[-25pt] 1 &[-25pt] &[-25pt] &[-25pt] &[-25pt] &[-25pt] &[-25pt] &[-25pt] &[-25pt] 1 &[-25pt] &[-25pt] &[-25pt]\\
	& & h^{0,1} & & h^{1,0} & & & & & & h^{0,1} & & h^{1,0} & & \\
	& h^{0,2} & & h^{1,1} & & h^{2,0} & & \rightsquigarrow & & h^{2,0} & & h^{1,1} +1 & & h^{0,2} & \\
	h^{0,3} & & h^{1,2} & & h^{2,1} & & h^{3,0} & & h^{0,3} & & h^{1,2}(+g) & & h^{2,1}(+g) & & h^{3,0} \\
    \end{tikzcd}
\end{center}

\begin{thm}
    Let $Y$ be a rational threefold.
    Then $J(Y)=J^{2}(Y)$ is isomorphic to a product of Jacobians of curves.
    \begin{proof}
	There exists $f\colon \P^{3}\dashrightarrow Y$ birational.
	\begin{center}
	    \begin{tikzcd}
		& \tilde{Z}\arrow[bend right=20]{ddl}\arrow{d}\arrow[bend left=20]{ddr} & \\
		& Z=\bar{\Gamma}_{f}\arrow{dl}\arrow{dr} & \\
		\P^{3} & & Y
	    \end{tikzcd}
	\end{center}
	By weak factorization, we have a chain
	\begin{center}
	    \begin{tikzcd}
		& Z_{1}'\arrow{dr}\arrow{dl} & & Z_{3}'\arrow{dl} & \cdots & Z_{n}'\arrow{dr} & \\
		\P^{3} & & Z_{2}' & & \cdots & & Y
	    \end{tikzcd}
	\end{center}
	Then we are done modulo the lemma below.
    \end{proof}
\end{thm}

\begin{lm}
    Let $f\colon Z'\to Z$ be a blowup map between smooth projective threefolds in a smooth point or curve.
    Then $J(Z')$ is a product of Jacobians of curves iff $J(Z)$ is.
    \begin{proof}
	If we blowup a point, then $H^{3}(Z',\Z)\cong H^{3}(Z,\Z)$.
	If we blowup a smooth curve $C$ of genus $g$, then 
	\[ H^{3}(Z',\Z)\cong H^{3}(Z,\Z)\op H^{1}(C,\Z). \]
    \end{proof}
\end{lm}

\begin{exa}
    The \textit{Klein cubic threefold} $X$ is given by
    \[ \sum_{i\in \Z/5\Z} x_{i}^{2}x_{i+1}. \]
    Let $\zeta$ be a primitive $11$-th root of unity in $\C$.
    We get automorphisms
    \[ \delta \colon (x_{0},\ldots,x_{4})\longmapsto (x_{0},\zeta x_{1},\zeta^{-1}x_{2},\zeta^{3} x_{3},\zeta^{6} x_{4}) \]
    and
    \[ \sigma \colon (x_{0},\ldots,x_{4})\longmapsto (x_{1},x_{2},x_{3}x_{4},x_{0}) \]
    which satisfy
    \[ \delta^{11}=1=\sigma^{5} \quad \text{ and }\quad \sigma \delta \sigma^{-1}=\delta^{-2}. \]
    $\delta$ has $5$ fixed points corresponding to the $5$ standard basis vectors and $\sigma $ has $4$ fixed points
    \[ (1:\alpha:\alpha^{2}:\alpha^{3}:\alpha^{4}) \]
    for $\alpha\in \C$, $\alpha\neq 1$, $\alpha^{5}=1$.
    Note that $h^{2i}(X,\Q)$ is $1$.
    These automorphisms have odd order, so $\delta^{*}$ and $\sigma^{*}$ must act trivially on $H^{2i}(X,\Q)$, and in odd cohomology, we only have something on $H^{3}$.
    We compute the trace of these isomorphisms acting on $H^{3}$:
    \[ \operatorname{tr}(\delta^{*}|_{H^{3}})=4-5=-1 \quad \text{ and }\quad \operatorname{tr}(\sigma^{*}|_{H^{3}})=4-4=0. \]
    These trace computations show that $\delta^{*}$ cannot act as the identity.

    Suppose now that $X$ is rational.
    Then $J(X)$ is a product of Jacobians of curves
    \[ J(X)=J(C_{1})\times \cdots \times J(C_{n}) \]
    with $n\leqslant 5$, because $J(X)$ is $10$-dimensional.
    $\delta^{*}$ permutes these factors.
    But the order of $\delta$ is $11$ and we have at most $5$ curves, hence $\delta^{*}$ stabilizes each factor.
    But $\delta^{*}$ acts non-trivially, so it must act non-trivially on some $J(C_{i})$.
    So we have
    \[ \delta^{*}|_{J(C_{i})}\colon H^{1}(C_{i},\Q)\to H^{1}(C_{i},\Q) \]
    with $\Q[<\delta^{*} >]=\Q\times \Q(\zeta)$.
    Hence $H^{1}(C_{i},\Q)$ is at least $10$-dimensional.
    This argument shows that we can only have one curve.

    Write $n=1$ and $C=C_{1}$, which is a curve of genus $5$ because its $H^{1}$ is $10$-dimensional.
    For curves of genus at least $2$, the Torelli theorem gives us a short exact sequence
    \[ 1\to \Aut(C)\to \Aut(J(C),\Theta)\to \begin{cases} \Z/2\Z &\text{ if }C \text{ hyperelliptic} \\ 1 &\text{ otherwise} \end{cases} \]
	Hence $\delta$ and $\sigma$ act on $C$.
	We can use the Lefschetz fixed point formula to compute the fixed points: $\delta$ has $3$ fixed points on $C$ and $\sigma$ has $2$ fixed points on $C$.
	Since $\sigma$ normalizes $<\delta >$, $\sigma$ must map $\operatorname{Fix}(\delta)$ to itself.
	But $\sigma$ has order $5$, so it must fix all these $3$ fixed points.
	This gives the desired contradiction, showing that $X$ is not rational.
\end{exa}

\section{[FS] Fabrizio Barroeiro - On the Zilber-Pink conjecture for complex abelian varieties - 14.02.20}

Joint work with G. Dill.
Varieties are irreducible.
$K$ is an algebraically closed field of characteristic zero.
$A/K$ will denote an abelian variety, $G/K$ a commutative algebraic group.

Abelinan varieties are very nice because they are always isogenous to products of powers of simple factors $A_{i}$ (abelian varieties without proper abelian subvarieties), so we have a factorization as in the integers.

\begin{exa}
    Let $E^{n}(K)\ni (P_{1},\ldots,P_{n})$, where $E$ is an elliptic curve,
    \[ \sum_{j=1}^{n}a_{ij}P_{i}=0 \]
    for $a_{ij}\in \End(E)$ yields a subgroup.
    The irreducible components of algebraic subgroups are of the form $\sum a_{ij}P_{i}=T$, for $T\in E_{tor}(K)$.
    For instance, $2P_{1}+2P_{2}=0$ has irreducible components $P_{1}+P_{2}=T$.
\end{exa}

\begin{defn}
    An irreducible component of an algebraic subgroup is called a \textit{special subvariety}.
    These are precisely translates of abelian subvarieties by torsion points.
\end{defn}

\begin{defn}
    A \textit{weakly special} subvariety is a translate of an abelian subvariety.
\end{defn}

\begin{q}[Lang '60]
    Take a curve $C\sub \C^{2}$ irreducible and containing infinitely many points $(\zeta_{1},\zeta_{2})$ with $\zeta_{i}$ roots of $1$.
    What can we say about this curve?
\end{q}

For example, $x^{a}y^{b}=\zeta$ for $\zeta$ a root of $1$ is such a curve.
Ihara--Serre--Tate proved that these are the only examples.
Notice how these curves are precisely special subvarieties of $\Gm^{2}=(\C^{\times})$.

\begin{thm}[Raynaud (him, not her) '83, previously conjectured by Manin-Mumford]
    A subvariety $V$ of an abelian variety $A$ contains at most finitely many maximal special subvarieties.
\end{thm}

\begin{nota}
    $V\sub A$, $<V>$ denotes the smallest special subvariety containing $V$, and $<V>_{ws}$ the smallest weakly special subvarieties containing $V$
\end{nota}

\begin{thm}
    Let $C\sub G$ irreducible curve over $K$ (to be specified later) and $\operatorname{HP}(C)$ some hypothesis on the curve (to be specified later).
    Let $\dim{G}=g$.
    Then 
    \[ \Sigma=\{ P\in C(K) \mid \dim(<P>)\leqslant g-2\} \]
    is finite.
\end{thm}

Let's now specify the hypotheses.
\begin{center}
    \begin{tabular}{|c|c|c|c}
	$G$ & $K$ & $\operatorname{HP}(C)$ & \\
	$\Gm^{n}$ & $\bar{\Q}$ & $<C>_{ws}=G$ & (Bombieri-Masser-Zannier '99) \\
	$\Gm^{n}$ & $\C$ & $<C>_{ws}=G$ & (BMZ '03) \\
	$\Gm^{n}$ & $\bar{\Q}$ & $<C>=G$ & (Maurim '08) \\
	$\Gm^{n}$ & $\C$ & $<C>=G$ & (BMZ '08) \\
	Some $A$ & $\bar{\Q}$ & $<C>=G$ & (VRRGC) \\
	$A$ & $\bar{\Q}$ & $<C>=G$ & (Habegger-Pila '16) \\
	$A$ & $\C$ & $<C>=G$ & (B-Dill '19) \\
	Semiab. & $\bar{\Q}$ & $<C>=G$ & (B-Kuehne-Schmidt)
    \end{tabular}
\end{center}

\begin{defn}
    Let $W\sub A$.
    The \textit{defect} of $W$ is defined as
    \[ \delta(W)=\dim(<W>)-\dim(W) \]
\end{defn}

In the theorem, $\delta(C)=g-1$, $P\in \Sigma \Leftrightarrow \delta(P)\leqslant g-2< \delta(C)$.

\begin{defn}
    Let $W\sub V\sub A$.
    We say that $W$ is \textit{optimal} for $V$ in $A$ if for all $U$ we have $W\subsetneq U\subseteq V$, $\delta(W)<\delta(V)$.
\end{defn}

Then the theorem says that there is at most finitely many optimal points.

\begin{conj}[Zilber-Pink]
    Fix $m,d\in \Z_{\geqslant 0}$.
    The statmement $\operatorname{ZP}(A,K,m,d)$ is: any $V\sub A$ with $\dim{V}\leqslant m$ contains at most finitely many optimal subvarieties of defect at most $d$.
\end{conj}

\begin{defn}
    We call $T\sub A$ the $K/\bar{\Q}$-trace of $A$ if $T$ is the largest abelian variety subvariety of $A$ that can be defined over $\bar{\Q}$.
\end{defn}

\begin{thm}[B.-Dill '19]
    $\operatorname{ZP}(T,\bar{\Q},m,d)\Rightarrow \operatorname{ZP}(A,K,m,d)$.
\end{thm}

HP'16 proved $\operatorname{ZP}(T,\bar{\Q},1,d)$ and $\operatorname{ZP}(T,\bar{\Q},n,1)$.
Combined, these yield $\operatorname{ZP}(T,\bar{\Q},m,d)$ if $\dim(T)\leqslant 4$.
Hence:

\begin{cor}
    $\operatorname{ZP}(A,K,m,d)$ holds if $m$ or $d$ are at most $1$ or $\dim{T}\leqslant 4$.
\end{cor}

\begin{exa}
    Take $A=E^{g}$ with transcendental $j$-invariant, which means that $E$ cannot be defined over $\bar{\Q}$.
    Hence the trace is trivial, so the conjecture holds in full generality for these kind of varieties.
\end{exa}

\begin{proof}[Sketch of proof]
    Can assume $K$ has finite transcendence degree over $\bar{\Q}$ and that $A$ is principally polarised.
    $A_{g,l}$ appropriate moduli space of ppav of dimension $g$.
    $\calA_{g,l}$ uniersal family over $A_{g,l}$.

    There exists $S\sub A_{g,l}$, $S/\bar{\Q}$ and an embedding $\bar{\Q}(S)\mono K$ such that $A\sim A'_{K}$ (isogeny) for $A'$ the generic fibre of $\calA_{g,l}\times_{A_{g,l}} S$.
    Take $K'=\overline{\Q(S)}$.
    Reductions:
    \begin{enumerate}
	\item $\operatorname{ZP}(A'_{K'},K',m,d)\Rightarrow \operatorname{ZP}(A,K,md)$.
	\item If $\dim{S}>0$ (if we have a genuiune family), then
	    \[ (\forall B \text{ proper quotient of }A',,\operatorname{ZP}(B,K',m,d))\Rightarrow \operatorname{ZP}(A',K',m,d) \]
    \end{enumerate}
\end{proof}
