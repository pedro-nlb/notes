...
\begin{lm}
    Let $F\colon \A\to \scrC$ be an exact functor which commutes with colimits (resp. colimits) indexed over $\N$.
    Then we can define $LF$ (resp. $RF$) in the whole derived category $\D(\A)$.
\end{lm}

Closed symmetric monoidal structure.
Example: $\Z[\X]\ot^{L}(-)=\Z[\X]\ot(-)$ is just degree-wise tensor product, because this functor is already exact so it need not be derived.

Triangulated structure and the problems it carries.
